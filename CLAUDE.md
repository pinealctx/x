...
以下目录和文件仅供本地使用，**禁止提交到 git 仓库**（已在 `.gitignore` 中配置）：
- `.process/` — 任务管理目录（进度、审计记录等）
- `.claude/` — Claude Code 配置（agents、skills、settings 等）
- `.vscode/` — 编辑器配置
- `.mcp.json` — MCP 服务器配置

如果这些文件已被 git 跟踪，使用 `git rm --cached -r <path>` 取消跟踪，本地文件保留。
