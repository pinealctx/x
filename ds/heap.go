...
func (h *Heap[T]) siftDown(i int) {
	n := len(h.data)
	for {
		chosen := i // index of the higher-priority node among i and its children
		left := 2*i + 1
		right := 2*i + 2
		if left < n && h.compare(h.data[left], h.data[chosen]) < 0 {
			chosen = left
		}
		if right < n && h.compare(h.data[right], h.data[chosen]) < 0 {
			chosen = right
		}
		if chosen == i {
			return
		}
		h.data[i], h.data[chosen] = h.data[chosen], h.data[i]
		i = chosen
	}
}
